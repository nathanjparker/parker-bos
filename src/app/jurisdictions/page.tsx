...
export default function JurisdictionsPage() {
  ...
  async function handleDelete(j: Jurisdiction) {
    if (
      !window.confirm(
        `Delete jurisdiction "${j.name}"? This cannot be undone.`
      )
    )
      return;
    setDeletingId(j.id);
    try {
      await deleteDoc(doc(db, "jurisdictions", j.id));
    } catch (err) {
      console.error("Delete jurisdiction error:", err);
        ...
        <div className="mt-4 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          {loading ? (
            ...
            <ul className="divide-y divide-gray-100">
              {filtered.map((j) => (
                <li key={j.id} className="flex items-start gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                  <Link href={`/jurisdictions/${j.id}`} className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-500">
                      {j.phone && <span>{formatPhoneDisplay(j.phone)}</span>}
                      {j.inspectionPhone && (
